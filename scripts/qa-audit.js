...
/* ── G9 외부 링크 0 · 광고 표기 ── */
{
  ...
  for (const [nm, h] of [...VENUES.map(v => [v.slug, docs[v.slug]]), ['hub', hub]]) {
    const hrefs = tag(h, /href="([^"]*)"/g)
      .filter(x => /^https?:\/\//.test(x) && !x.startsWith('https://b.nolcool.com'));
...
/* ── G10 전화 허용표 (홈=춘자 / 광고주 4곳만 자기 담당) ── */
{
  ...
  for (const v of VENUES) {
    ...
    const names = ['춘자', '손흥민', '미제휴', '펩시맨'].filter(n => docs[v.slug].includes(n));
    if (ADV[v.name]) {
  ...
  const homeOther = ['손흥민', '미제휴', '펩시맨'].filter(n => home.includes(n));
